...
new Swiper('.slider-intoducing__body', {
	wrapperClass: 'slider-intoducing__wrapper',
	slideClass: 'slider-intoducing__item',
	observer: true,
	observeParents: true,
	slidesPerView: 1,
	spaceBetween: 0,
...
const currentSlideLength = document.querySelectorAll('.slider-our-spaces__slide').length;

const sliderOurSpaces = new Swiper('.slider-our-spaces__body', {
	observer: true,
	observeParents: true,
	slidesPerView: 3,
	spaceBetween: 30,
	...
	on: {
		...
		init: function () {
			allSlides.innerHTML = setZeroFormat(currentSlideLength);
		},
...
function initOurFacilitiesSlider() {
	...
	if (sliderBlocks.length > 0) {
		sliderBlocks.forEach((sliderBlock, index) => {
			...
			const allOurFacilitiesSliders = document.querySelectorAll('.contols-our-facilities__item')[index].querySelector('._fraction__all');
			const allCurrentOurFacilitiesSlide = document.querySelectorAll('.contols-our-facilities__item')[index].querySelector('._fraction__current');
			const allOurFacilitiesSlidersProgress = document.querySelectorAll('.contols-our-facilities__item')[index].querySelector('.contols-our-facilities__progress');
...
tabsOurFacilities.addEventListener('click', function (e) {
	...
	if (targetElement.classList.contains('tabs-our-facilities__item')) {
		...
		const OurFacilitiesControls = document.querySelectorAll('.contols-our-facilities__item');
...
help.addEventListener('click', function (e) {
	...
	if (targetItem.classList.contains('help__link')) {
		_slideToggle(targetItem.previousElementSibling);
		targetItem.classList.toggle('_active');
		e.preventDefault();
	}
	if (targetItem.classList.contains('help__more')) {
		if (!targetItem.classList.contains('_loading')) {
			targetItem.classList.add('_loading');
			getMoreQuestions(targetItem);
		}
		e.preventDefault();
	}
});

async function getMoreQuestions(targetItem) {
	const response = await fetch(
		'https://gist.githubusercontent.com/alexey1985-FE/87a321c01de36052b7dcd241e5067011/raw/b8650945843bd0087ecd035cf284597d9e37089f/faq.html'
	);
	if (response.ok) {
		const text = await response.text();
		let helpItems = document.querySelector('.help__items');
		helpItems.insertAdjacentHTML('beforeend', text);
		targetItem.classList.remove('_loading');
	} else {
		alert('Что-то не так!!!');
	}
}
